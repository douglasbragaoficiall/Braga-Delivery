...
class Myd_Custom_Fields {
	...
	public function render_input_image( array $args, int $post_id, string $value = '' ) {
		...
		return sprintf(
			...
			$image_preview,
			$image_input,
			esc_html__( 'Choose', 'drope-delivery' ),
			esc_html__( 'Remove', 'drope-delivery' )
		);
	}
	...
	public function render_input_repeater( array $args, int $post_id, $value = '' ) {
		...
		?>
			<div class="myd-repeater-wrapper" id="<?php echo esc_attr( $item_id ); ?>">
				...
				<button href="#" class="button button-primary myd-repeater-add-extra" id="myd-repeater-add-extra" data-row="<?php echo esc_attr( $item_id ); ?>"><?php esc_html_e( 'Add product extra', 'drope-delivery' ); ?></button>
			</div>
		<?php
			...
			<div class="myd-repeater-container myd-repeater-container--top-level">
				...
				<?php foreach ( $fields as $key => $field ) : ?>
					...
					<?php else : ?>
							<?php foreach ( $field as $internal_key => $internal_field ) : ?>
								<details class="myd-repeater-container myd-repeater-container--internal" data-index="<?php echo esc_attr( $key ); ?>">
									<summary class="myd-repeater-summary">
										...
										<span class="myd-repeater-summary__action--remove" data-row="<?php echo esc_attr( $item_id ); ?>"><?php esc_html_e( 'remove', 'drope-delivery' ); ?></span>
									</summary>
							...
							<button href="#" class="button button-secondary myd-extra-option-button myd-repeater-add-option" id="myd-repeater-add-option"><?php _e( 'Add option', 'drope-delivery' ); ?></button>
					<?php endif; ?>
				<?php endforeach; ?>
			</div>
		<?php
	...
	public function render_input_select( array $args, int $post_id, string $value = '' ) {
		...
		return sprintf(
			...
			esc_attr( $required ),
			isset( $args['data'] ) ? $this->build_data_attr( $args['data'] ) : '',
			esc_html__( 'Select', 'drope-delivery' ),
			implode( $options )
		);
	}
