...
class License {
    public $key = "C2118F73DF2D3FD9";
    private $product_id = "1";
    private $product_base = "drope-delivery";
    private $server_host = "https://veroapp.com.br/wp-json/licensor/";
    ...
    function __construct($plugin_base_file='')
    {
        ...
        if($this->has_check_update) {
            if(function_exists("add_action")){
                add_action( 'admin_post_drope-delivery_fupc', function(){
            ...
            if(function_exists("add_filter")) {
                if($this->is_theme){
                    ...
                    add_action('admin_menu',function(){
                        add_theme_page( 'Verificar atualizações', 'Verificar atualizações', 'edit_theme_options', 'update_check', [$this,"theme_force_update"] );
                    },999);
                }else{
                    ...
                    add_filter( 'plugin_row_meta', function($links, $plugin_file ){
                        if (  plugin_basename( $this->plugin_file ) == $plugin_file ) {
                            $links[] = " <a class='edit coption' href='" . esc_url( admin_url( 'admin-post.php' ) . '?action=drope-delivery_fupc' ) . "'>Verificar atualizações</a>";
                        }
                        return $links;
                    }, 10, 2 );
     ...
     public function theme_force_update(){
        ...
        echo wp_kses_post('<h1>'.__("Verificar atualizaçõesing..","drope-delivery").'</h1>');
    ...
    public function update_message_cb($data, $response){
        ...
        if(isset($data->package) && empty($data->package)) {
            if(empty($data->update_denied_type)) {
                print  "<br/><span style='display: block; border-top: 1px solid #ccc;padding-top: 5px; margin-top: 10px;'>Por favor <strong>ative sua licença</strong> ou  <strong>renove seu período de suporte</strong> para obter a versão mais recente do plugin.</span>";
            }elseif("L" == $data->update_denied_type){
                print  "<br/><span style='display: block; border-top: 1px solid #ccc;padding-top: 5px; margin-top: 10px;'>Por favor <strong>ative sua licença</strong> para obter a versão mais recente do plugin.</span>";
            }elseif("S" == $data->update_denied_type){
                print  "<br/><span style='display: block; border-top: 1px solid #ccc;padding-top: 5px; margin-top: 10px;'>Por favor <strong>renove seu período de suporte</strong> para obter a versão mais recente do plugin.</span>";
            }
        }
    }
